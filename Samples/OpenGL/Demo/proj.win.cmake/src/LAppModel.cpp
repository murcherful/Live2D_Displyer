...
void LAppModel::Update()
{
    ...
    {
		POINT last_p = LAppDelegate::GetInstance()->last_p;
		POINT p = LAppDelegate::GetInstance()->p;
        // モーションの再生がない場合、待機モーションの中からランダムで再生する
		if (last_p.x == p.x && last_p.y == p.y) {
			if (!_isIdle) {
				_isIdle = 1;
				_my_idle_start_time = ::GetTickCount();
			}
			else if (::GetTickCount() - _my_idle_start_time >= 3000) {
				printf("<MCF>: get into idle state\n");
				StartRandomMotion(MotionGroupIdle, PriorityIdle);
			}
		}
		else {
			_isIdle = 0;
			//printf("<MCF>: mouse moving\n");
		}
    }
    else
    {
