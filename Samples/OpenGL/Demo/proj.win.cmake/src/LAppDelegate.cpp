...
bool LAppDelegate::Initialize()
{
	...
	GLFWimage images[2]; 
	images[0].pixels = stbi_load(strcat(RP, "icon.png"), &images[0].width, &images[0].height, 0, 4);
	strcpy(RP, LAppDefine::ResourcesPath);
	images[1].pixels = stbi_load(strcat(RP, "icon_small.png"), &images[1].width, &images[1].height, 0, 4);
...
void LAppDelegate::Run()
{
	...
	while (glfwWindowShouldClose(_window) == GL_FALSE && !_isEnd)
	{
		...
		glfwGetWindowSize(LAppDelegate::GetInstance()->GetWindow(), &width, &height);
		int xpos, ypos;
		glfwGetWindowPos(LAppDelegate::GetInstance()->GetWindow(), &xpos, &ypos);
		...
		else if (p.x != last_p.x || p.y != last_p.y) {
			_view->OnTouchesMoved((int)p.x - xpos, (int)p.y - ypos);
			//printf("%d, %d\n", (int)p.x - xpos, (int)p.y - ypos);
			//printf("%lld, %lld, %d, %d\n", p.x, p.y, xpos, ypos);
		}
